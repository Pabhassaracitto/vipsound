...
class _TipitakaReaderScreenState extends State<TipitakaReaderScreen> {
  ...
  Future<void> _loadFirstPage() async {
    ...
    try {
      ...
      final requestedOffset = widget.initialSegmentId == null
          ? 0
          : (await TipitakaDb.getSegmentOrderIndex(db, widget.initialSegmentId!) ?? 0)
  ...
  Future<void> _saveSelection(
    ...
    try {
      ...
      final result = await const TipitakaWorklistService().saveSelection(
        ...
        endOffset: endOffset,
        translationLanguage: segment.translationVi?.trim().isNotEmpty == true
            ? 'vi'
            : 'en',
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            if (index <= _segments.length) {
              return _SegmentCard(
                key: widget.initialSegmentId == _segments[index - 1].id
                    ? _initialSegmentKey
